...
st.caption("Next-month Aadhaar service stress alerts with explanations & actions")
...
with tab_alerts:
    ...
    with tab_high:
        ...
        st.dataframe(
            df[["state", "district", "Risk Level", "Recommended Action"]]
...
with tab_analysis:
    ...
    st.markdown(
        f"### {selected_state} → {selected_district} : **{risk_map[risk_level]} Risk Next Month**"
    )
